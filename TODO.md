# Backend
1. Migrate to MySQL
2. [COMPLETED] Change all errors to custom error using Errors.New() rather than using string comparisons. 
...
19) Make it so the DB stores the MIME type of the file when initally recieving and storing it, so it can be later used for filtering and giving the user info when multiple kinds of file are stored in the DB
